...
export function lambdaApiInvoker(arn: string, lambda: Lambda, transform: (request: APIGatewayProxyEvent) => APIGatewayProxyEvent = r => r): Caller {
  return { call: (async (method, resource, path, body, pathParameters, queryParameters, multiQueryParameters, headers) => {
      const request = transform({ httpMethod: method, resource, path, body: body ?? null, pathParameters: pathParameters as any, headers: headers as any, queryStringParameters: queryParameters, multiValueQueryStringParameters: multiQueryParameters} as APIGatewayProxyEvent);
